...
<div class="mt-3 px-[11px] pr-[10px]">
    ...
    <div class="!z-5 relative flex flex-col rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none p-6">
        ...
        <div class="mb-6 flex items-center justify-between border-b border-gray-200 dark:border-white/10 pb-4">
            <div>
                <h4 class="text-xl font-bold text-navy-700 dark:text-white">Edit App Version</h4>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Update app version information for {{ $appVersion->platform }}</p>
            </div>
        ...
        <form action="{{ route('app-version.update', $appVersion->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-5">
                ...
                <div class="flex items-start gap-3 rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-navy-900/50 p-4">
                    <input 
                        ...
                        value="1"
                        {{ old('maintenance_mode', $appVersion->maintenance_mode) ? 'checked' : '' }}
                ...
                <div>
                    ...
                    >{{ old('maintenance_message', $appVersion->maintenance_message) }}</textarea>
                ...
                <div>
                    ...
                    <input 
                        ...
                        id="store_url"
                        name="store_url" 
                        value="{{ old('store_url', $appVersion->store_url) }}"
                ...
                <!-- Timestamps -->
                <div class="grid grid-cols-2 gap-4 rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-navy-900/50 p-4">
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-600">Created</p>
                        <p class="mt-1 text-sm font-bold text-navy-700 dark:text-white">{{ $appVersion->created_at->format('d M Y H:i') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-600">Updated</p>
                        <p class="mt-1 text-sm font-bold text-navy-700 dark:text-white">{{ $appVersion->updated_at->format('d M Y H:i') }}</p>
                    </div>
                </div>
            </div>
        ...
        <!-- Delete Form (Hidden) -->
        <form id="deleteForm" action="{{ route('app-version.destroy', $appVersion->id) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('button[onclick*="deleteForm"]').addEventListener('click', function(e) {
        e.preventDefault();
        if (confirm('Apakah Anda yakin ingin menghapus app version ini? Tindakan ini tidak dapat dibatalkan.')) {
