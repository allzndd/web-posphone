...
<div class="mt-3 px-[11px] pr-[10px]">
    <div class="!z-5 relative flex flex-col rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none p-6">
        <h4 class="text-xl font-bold text-navy-700 dark:text-white mb-6">Edit Service</h4>
        
        <form action="{{ route('langganan.update', $item->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <select name="tipe_layanan_id" id="tipe_layanan_id" required
                        ...
                        @foreach($paketLayanan as $paket)
                            <option value="{{ $paket->id }}" 
                                    ...
                                    {{ old('tipe_layanan_id', $item->tipe_layanan_id) == $paket->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date" name="started_date" id="started_date" value="{{ old('started_date', $item->started_date->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $item->end_date->format('Y-m-d')) }}" required
                    ...
                    @error('end_date')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Can be manually adjusted or auto-calculated</p>
                </div>
                
                <div class="md:col-span-2">
                    <div class="flex gap-6">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="is_trial" value="1" {{ old('is_trial', $item->is_trial) ? 'checked' : '' }}
