...
<div class="mt-3 px-[11px] pr-[10px]">
    <div class="!z-5 relative flex flex-col rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none p-6">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div>
                <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Service ID</p>
                <p class="mt-1 text-base font-bold text-navy-700 dark:text-white">#{{ str_pad($item->id, 5, '0', STR_PAD_LEFT) }}</p>
            </div>
            
            <div>
                <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Service Name</p>
                <p class="mt-1 text-base font-bold text-navy-700 dark:text-white">{{ $item->nama }}</p>
            </div>
            
            <div class="md:col-span-2">
                <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Description</p>
                <p class="mt-1 text-base text-navy-700 dark:text-white">{{ $item->deskripsi }}</p>
            </div>
            
            <div>
                <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Price</p>
                <p class="mt-1 text-base font-bold text-navy-700 dark:text-white">Rp {{ number_format($item->harga, 0, ',', '.') }}</p>
            </div>
