...
@section('main')
@include('components.access-denied-overlay', ['module' => 'Role', 'hasAccessRead' => $hasAccessRead])
            ...
            <div class="flex items-center gap-3">
                ...
                <form method="GET" action="{{ route('pos-role.index') }}" class="relative">
                    <div class="flex items-center rounded-xl border border-gray-200 dark:border-white/10 bg-lightPrimary dark:bg-navy-900 px-4 py-2">
                        ...
                        <input type="text" name="nama" value="{{ request('nama') }}" placeholder="Search roles..." 
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-200 dark:border-white/10">
                        ...
                        <th class="py-3 text-left">
                            <p class="text-sm font-bold text-gray-600 dark:text-white uppercase">Users Count</p>
                        </th>
                        <th class="py-3 text-left">
                    ...
                    <tr>
                        <td colspan="5" class="py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-lg font-medium text-gray-600 dark:text-gray-400">No roles found</p>
        ...
        <div class="border-t border-gray-200 dark:border-white/10 px-6 py-4">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-1">
                    ...
                    @else
                        <a href="{{ $roles->previousPageUrl() }}&per_page={{ request('per_page', 10) }}{{ request('nama') ? '&nama=' . request('nama') : '' }}" 
                    ...
                    @for ($page = max(1, $roles->currentPage() - 2); $page <= min($roles->lastPage(), $roles->currentPage() + 2); $page++)
                        ...
                        @else
                            <a href="{{ $roles->url($page) }}&per_page={{ request('per_page', 10) }}{{ request('nama') ? '&nama=' . request('nama') : '' }}" 
                    ...
                    @if ($roles->hasMorePages())
                        <a href="{{ $roles->nextPageUrl() }}&per_page={{ request('per_page', 10) }}{{ request('nama') ? '&nama=' . request('nama') : '' }}" 
                           class="flex h-9 w-9 items-center justify-center rounded-lg bg-lightPrimary text-brand-500 transition duration-200 hover:bg-gray-100 dark:bg-navy-700 dark:text-white">▶</a>
                    @else
                        <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-lightPrimary text-gray-400 dark:bg-navy-700 dark:text-gray-600 cursor-not-allowed">▶</span>
                    @endif
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    editMenuItem.addEventListener('click', function(e) {
        ...
        if (currentButton) {
            const editUrl = currentButton.getAttribute('data-role-edit');
            if (editUrl) {
    ...
    deleteMenuItem.addEventListener('click', function(e) {
        ...
        if (currentButton) {
            const destroyUrl = currentButton.getAttribute('data-role-destroy');
            if (destroyUrl) {
                const modal = document.getElementById('deleteConfirmModal');
                const messageEl = modal.querySelector('p.text-gray-600');
                messageEl.innerHTML = 'Apakah Anda yakin ingin menghapus role ini?';
                modal.classList.remove('hidden');
