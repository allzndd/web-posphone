...
<div class="mt-3 px-[11px] pr-[10px]">
    ...
    <div class="!z-5 relative flex flex-col rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none">
        ...
        <div class="border-b border-gray-200 dark:border-white/10 p-6">
            <h4 class="text-xl font-bold text-navy-700 dark:text-white">Edit Stock</h4>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Update product stock information</p>
        </div>

        <!-- Form -->
        <form action="{{ route('produk-stok.update', $produkStok->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="pos_produk_id" required
                        ...
                        @foreach($produk as $item)
                            <option value="{{ $item->id }}" {{ old('pos_produk_id', $produkStok->pos_produk_id) == $item->id ? 'selected' : '' }}>
                                {{ $item->display_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="pos_toko_id" required
                        ...
                        @foreach($toko as $item)
                            <option value="{{ $item->id }}" {{ old('pos_toko_id', $produkStok->pos_toko_id) == $item->id ? 'selected' : '' }}>
                                {{ $item->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="stok" value="{{ old('stok', $produkStok->stok) }}" required min="0"
