    ...
    <div class="!z-5 relative flex flex-col rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between border-t border-gray-200 dark:border-white/10 px-6 py-4 gap-4">
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <form method="GET" action="{{ route('toko.index') }}" id="perPageForm" class="inline-block">
                ...
                <span class="text-sm text-gray-600 dark:text-gray-400">
                    Showing {{ $tokos->firstItem() ?? 0 }} to {{ $tokos->lastItem() ?? 0 }} of {{ $tokos->total() }} results
                </span>
            </div>
            <div class="flex items-center gap-1">
                ...
                {{-- Page Numbers --}}
                @foreach ($tokos->getUrlRange(max(1, $tokos->currentPage() - 2), min($tokos->lastPage(), $tokos->currentPage() + 2)) as $page => $url)
