...
<div class="mt-3 px-[11px] pr-[10px]">
    ...
    <div class="!z-5 relative flex flex-col rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none p-6">
        ...
        <form action="{{ route('pos-produk-merk.update', $merk->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-5">
                ...
                <div class="rounded-xl border border-gray-200 dark:border-white/10 bg-lightPrimary dark:bg-navy-900 p-4">
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        ...
                        <div>
                            <p class="text-xs text-gray-500 dark:text-gray-600">Products with this brand</p>
                ...
                <div class="rounded-xl border border-yellow-200 dark:border-yellow-800/30 bg-yellow-50 dark:bg-yellow-900/20 p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-sm font-bold text-yellow-900 dark:text-yellow-300">Active Products</p>
                            <p class="mt-1 text-xs text-yellow-800 dark:text-yellow-400">
                                This brand is currently assigned to {{ $merk->produk_count }} product(s). Changing the brand name will update it for all assigned products.
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <!-- Delete Form (Hidden) -->
        <form id="deleteForm" action="{{ route('pos-produk-merk.destroy', $merk->id) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('button[onclick*="deleteForm"]').addEventListener('click', function(e) {
        e.preventDefault();
        if (confirm('Apakah Anda yakin ingin menghapus brand ini? Tindakan ini tidak dapat dibatalkan.')) {
