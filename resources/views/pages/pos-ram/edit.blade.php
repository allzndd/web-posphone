...
<div class="mt-3 px-[11px] pr-[10px]">
    ...
    <div class="!z-5 relative flex flex-col rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none p-6">
        ...
        <div class="mb-6 flex items-center justify-between border-b border-gray-200 dark:border-white/10 pb-4">
            <div>
                <h4 class="text-xl font-bold text-navy-700 dark:text-white">Edit RAM</h4>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Update informasi kapasitas RAM</p>
            </div>
        ...
        <form action="{{ route('pos-ram.update', $posRam) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-5">
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($owners as $owner)
                                <option value="{{ $owner->id }}" {{ old('id_owner', $posRam->id_owner) == $owner->id ? 'selected' : '' }}>
                        ...
                        @enderror
                        <p class="mt-2 text-xs text-gray-500 dark:text-gray-600">Pilih owner yang akan mengelola RAM ini</p>
                    </div>
                @else
                ...
                <div>
                    ...
                    <input 
                        ...
                        id="kapasitas"
                        name="kapasitas" 
                        value="{{ old('kapasitas', $posRam->kapasitas) }}"
                ...
                <div class="rounded-xl border border-gray-200 dark:border-white/10 bg-lightPrimary dark:bg-navy-900 p-4">
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        <div>
                            ...
                            <p class="mt-1 text-sm font-bold text-navy-700 dark:text-white">{{ $posRam->created_at->format('d/m/Y H:i:s') }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-bold text-gray-600 dark:text-gray-400 uppercase">Diperbarui</p>
                            <p class="mt-1 text-sm font-bold text-navy-700 dark:text-white">{{ $posRam->updated_at->format('d/m/Y H:i:s') }}</p>
                        </div>
                    </div>
                </div>
        ...
        <!-- Delete Form (Hidden) -->
        <form id="deleteForm" action="{{ route('pos-ram.destroy', $posRam) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('button[onclick*="deleteForm"]').addEventListener('click', function(e) {
        e.preventDefault();
        if (confirm('Apakah Anda yakin ingin menghapus RAM ini? Tindakan ini tidak dapat dibatalkan.')) {
