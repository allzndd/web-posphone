...
<div class="mt-3 px-[11px] pr-[10px]">
    <!-- Product Names Table Card -->
    <div class="!z-5 relative flex flex-col rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none">
        <!-- Card Header -->
        <div class="flex items-center justify-between p-6 pb-4">
            <div>
                <h4 class="text-xl font-bold text-navy-700 dark:text-white">All Product Names</h4>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    {{ $names->total() }} total product names
                </p>
            </div>
            ...
            <div class="flex items-center gap-3">
                <!-- Search Form -->
                <form method="GET" action="{{ route('product-name.index') }}" class="relative">
                    <div class="flex items-center rounded-xl border border-gray-200 dark:border-white/10 bg-lightPrimary dark:bg-navy-900 px-4 py-2">
                        ...
                        <input type="text" name="name" value="{{ request('name') }}" placeholder="Search product names..." 
                ...
                <!-- Add New Button -->
                <a href="{{ route('product-name.create') }}" 
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-200 dark:border-white/10">
                        ...
                        <th class="py-3 text-left">
                            <p class="text-sm font-bold text-gray-600 dark:text-white uppercase">Slug</p>
                        </th>
                        <th class="py-3 text-left">
                            <p class="text-sm font-bold text-gray-600 dark:text-white uppercase">Description</p>
                        </th>
                        <th class="py-3 text-left">
                            <p class="text-sm font-bold text-gray-600 dark:text-white uppercase">Total Stock</p>
                        </th>
                ...
                <tbody>
                    @forelse ($names as $n)
                    <tr class="border-b border-gray-100 dark:border-white/10 hover:bg-lightPrimary dark:hover:bg-navy-700 transition-colors cursor-pointer" data-href="{{ route('product-name.edit', $n->id) }}">
                        <td class="py-4">
                            <p class="text-sm font-bold text-navy-700 dark:text-white">{{ $n->name }}</p>
                        </td>
                        <td class="py-4">
                            <p class="text-sm text-gray-600 dark:text-gray-400">{{ $n->slug }}</p>
                        </td>
                        <td class="py-4">
                            <p class="text-sm text-gray-600 dark:text-gray-400">{{ $n->description ?? '-' }}</p>
                        </td>
                        <td class="py-4">
                            @php $totalStock = $stocks[$n->name] ?? 0; @endphp
                            @if($totalStock == 0)
                        ...
                        <td class="py-4" onclick="event.stopPropagation()">
                            <div class="flex items-center justify-center gap-2">
                                <!-- Edit Button -->
                                <a href="{{ route('product-name.edit', $n->id) }}" 
                                ...
                                <!-- Delete Button -->
                                <form action="{{ route('product-name.destroy', $n->id) }}" method="POST" class="inline-block" 
                                      onsubmit="return confirm('Are you sure you want to delete this product name? If it\'s being used by any products, deletion will be blocked.');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                    ...
                    <tr>
                        <td colspan="5" class="py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-lg font-medium text-gray-600 dark:text-gray-400">No product names found</p>
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between border-t border-gray-200 dark:border-white/10 px-6 py-4 gap-4">
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <form method="GET" action="{{ route('product-name.index') }}" id="perPageForm" class="inline-block">
                ...
                <span class="text-sm text-gray-600 dark:text-gray-400">
                    Showing {{ $names->firstItem() ?? 0 }} to {{ $names->lastItem() ?? 0 }} of {{ $names->total() }} results
                </span>
            </div>
            <div class="flex items-center gap-1">
                ...
                @else
                    <a href="{{ $names->previousPageUrl() }}&per_page={{ request('per_page', 10) }}" 
                       class="flex h-9 w-9 items-center justify-center rounded-lg bg-lightPrimary text-brand-500 transition duration-200 hover:bg-gray-100 dark:bg-navy-700 dark:text-white dark:hover:bg-white/20">
                        <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" class="h-5 w-5" xmlns="http://www.w3.org/2000/svg">
                            <path fill="none" d="M0 0h24v24H0z"></path>
                            <path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"></path>
                ...
                @if ($names->hasMorePages())
                    <a href="{{ $names->nextPageUrl() }}&per_page={{ request('per_page', 10) }}" 
                       class="flex h-9 w-9 items-center justify-center rounded-lg bg-lightPrimary text-brand-500 transition duration-200 hover:bg-gray-100 dark:bg-navy-700 dark:text-white dark:hover:bg-white/20">
                        <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" class="h-5 w-5" xmlns="http://www.w3.org/2000/svg">
                            <path fill="none" d="M0 0h24v24H0z"></path>
