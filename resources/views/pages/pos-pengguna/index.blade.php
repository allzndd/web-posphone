...
<div class="mt-3 px-[11px] pr-[10px]">
    <!-- POS Users Table Card -->
    <div class="!z-5 relative flex flex-col rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none">
        <!-- Card Header -->
        <div class="flex items-center justify-between p-6 pb-4">
            <div>
                <h4 class="text-xl font-bold text-navy-700 dark:text-white">POS Users</h4>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    {{ $pengguna->total() }} total users
                </p>
            </div>
            ...
            <div class="flex items-center gap-3">
                <!-- Search Form -->
                <form method="GET" action="{{ route('pos-pengguna.index') }}" class="relative">
                    <div class="flex items-center rounded-xl border border-gray-200 dark:border-white/10 bg-lightPrimary dark:bg-navy-900 px-4 py-2">
                        ...
                        <input type="text" name="nama" value="{{ request('nama') }}" placeholder="Search users..." 
            ...
            <table class="w-full">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-100 dark:border-white/10 hover:bg-lightPrimary dark:hover:bg-navy-700 transition-colors cursor-pointer" data-href="{{ route('pos-pengguna.edit', $user->id) }}">
                        ...
                        <td class="py-4">
                            @if($user->toko)
                                <p class="text-sm text-gray-600 dark:text-gray-400">{{ $user->toko->nama }}</p>
                            @else
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between border-t border-gray-200 dark:border-white/10 px-6 py-4 gap-4">
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <form method="GET" action="{{ route('pos-pengguna.index') }}" id="perPageForm" class="inline-block">
                ...
                <span class="text-sm text-gray-600 dark:text-gray-400">
                    Showing {{ $pengguna->firstItem() ?? 0 }} to {{ $pengguna->lastItem() ?? 0 }} of {{ $pengguna->total() }} results
                </span>
            </div>
            <div class="flex items-center gap-1">
                ...
                {{-- Page Numbers --}}
                @foreach ($pengguna->getUrlRange(max(1, $pengguna->currentPage() - 2), min($pengguna->lastPage(), $pengguna->currentPage() + 2)) as $page => $url)
