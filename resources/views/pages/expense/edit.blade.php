...
<div class="mt-3 px-[11px] pr-[10px]">
    ...
    <div class="!z-5 relative flex flex-col rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none p-6">
        ...
        <div class="mb-6 flex items-center justify-between border-b border-gray-200 dark:border-white/10 pb-4">
            <div>
                <h4 class="text-xl font-bold text-navy-700 dark:text-white">Edit Expense</h4>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Update expense transaction #{{ $expense->invoice }}</p>
            </div>
        ...
        <form id="expenseForm" action="{{ route('expense.update', $expense) }}" method="POST">
            @csrf
            @method('PUT')
                        ...
                        <div class="relative">
                            ...
                            >
                                ...
                                @foreach($tokos as $toko)
                                    <option value="{{ $toko->id }}" {{ old('pos_toko_id', $expense->pos_toko_id) == $toko->id ? 'selected' : '' }}>
                                        {{ $toko->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="relative">
                            ...
                            >
                                ...
                                @foreach($kategoris as $kategori)
                                    <option value="{{ $kategori->id }}" {{ old('pos_kategori_expense_id', $expense->pos_kategori_expense_id) == $kategori->id ? 'selected' : '' }}>
                                        {{ $kategori->nama }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input 
                                ...
                                id="total_harga"
                                name="total_harga" 
                                value="{{ old('total_harga', $expense->total_harga) }}"
                                step="0.01"
                                min="0"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            >
                                ...
                                <option value="non tunai" {{ old('metode_pembayaran', $expense->metode_pembayaran) == 'non tunai' ? 'selected' : '' }}>Non Tunai</option>
                            </select>
                            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-4">
                                <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" class="h-5 w-5 text-gray-400 dark:text-gray-600" xmlns="http://www.w3.org/2000/svg">
                                    <path fill="none" d="M0 0h24v24H0z"></path>
                    ...
                    <div class="md:col-span-2">
                        ...
                        >{{ old('keterangan', $expense->keterangan) }}</textarea>
                        @error('keterangan')
                            <p class="mt-2 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="mb-6 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 p-4">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <h6 class="text-sm font-bold text-blue-900 dark:text-blue-300">Update Expense</h6>
                        <p class="mt-1 text-sm text-blue-700 dark:text-blue-400">
                            Make sure all information is correct before updating. Created at {{ $expense->created_at->format('d M Y H:i') }}.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 border-t border-gray-200 dark:border-white/10 pt-6">
                ...
                <button type="submit" 
                        id="submitBtn"
                        class="flex items-center gap-2 rounded-xl bg-blue-500 px-6 py-3 text-sm font-bold text-white transition duration-200 hover:bg-blue-600 active:bg-blue-700 dark:bg-blue-400 dark:hover:bg-blue-300 dark:active:bg-blue-200">
                    <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" class="h-5 w-5" xmlns="http://www.w3.org/2000/svg">
                        <path fill="none" d="M0 0h24v24H0z"></path>
                        <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"></path>
                    </svg>
                    <span id="submitBtnText">Update Expense</span>
                </button>
                        </div>
