...
<div id="sidebar" class="sidebar-custom-scroll sm:none duration-175 linear fixed !z-50 flex min-h-full max-h-screen flex-col bg-white pb-10 shadow-2xl shadow-white/5 transition-all dark:!bg-navy-800 dark:text-white md:!z-50 lg:!z-50 xl:!z-0 overflow-y-auto" :class="sidebarOpen ? 'translate-x-0' : '-translate-x-96 xl:translate-x-0'">
    ...
    <ul class="mb-auto pt-1">
        ...
        <li class="relative mb-3" x-data="{ open: {{ Request::is('pos-penyimpanan*') || Request::is('pos-warna*') ? 'true' : 'false' }} }">
            <div class="flex hover:cursor-pointer" @click="open = !open">
                <div class="w-full">
                    <div class="my-[3px] flex cursor-pointer items-center px-8">
                        <span class="{{ Request::is('pos-penyimpanan*') || Request::is('pos-warna*') ? 'font-bold text-brand-500 dark:text-white' : 'font-medium text-gray-600 dark:text-gray-400' }}">
                        ...
                        <p class="leading-1 ml-4 flex {{ Request::is('pos-penyimpanan*') || Request::is('pos-warna*') ? 'font-bold text-navy-700 dark:text-white' : 'font-medium text-gray-600 dark:text-gray-400' }}">
                            Data Master
                        </p>
                        <span class="ml-auto mr-3 text-gray-600 transition-transform duration-200" :class="{ 'rotate-180': open }">
                            <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" class="h-4 w-4" xmlns="http://www.w3.org/2000/svg"><path fill="none" d="M0 0h24v24H0z"></path><path d="M7 10l5 5 5-5z"></path></svg>
            ...
            @if(Request::is('pos-penyimpanan*') || Request::is('pos-warna*'))
                <div class="absolute right-0 top-px h-9 w-1 rounded-lg bg-brand-500 dark:bg-brand-400"></div>
            @endif
            <!-- Submenu -->
            <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 transform scale-y-95" x-transition:enter-end="opacity-100 transform scale-y-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 transform scale-y-100" x-transition:leave-end="opacity-0 transform scale-y-95" class="overflow-hidden" @if(!Request::is('pos-penyimpanan*') && !Request::is('pos-warna*')) style="display: none;" @endif>
                <ul class="my-[3px]">
                    <li class="relative mb-2 flex hover:cursor-pointer">
